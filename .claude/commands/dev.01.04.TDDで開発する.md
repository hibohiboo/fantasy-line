...
### 横断的一貫性チェック
**今書いたファイルだけでなく、同じパッケージ・ディレクトリにある既存ファイルも見ること。**
新しいパターン（ユーティリティ関数・テストヘルパー・スキーマ規約など）を導入した場合、既存コードが古いやり方のままになっていないか確認する。

確認項目:
- 同じユーティリティを全ファイルで使っているか（例: `json()`, `getOwnerId()` を追加したなら、他ハンドラーでも使われているか）
- テストのセットアップヘルパーが統一されているか（例: `useMysqlContainer` を追加したなら、他の Medium テストでも使われているか）
- DB スキーマのカラム定義が統一されているか（例: `.notNull()` の有無、`defaultNow()` の書き方）
- 同じテストケースが Small/Medium の両方に重複していないか
- `setTimeout` などスリープに依存したテストがないか（CI が遅くなる原因）
- 型アサーション（`as unknown as`）が過度に使われていないか

問題を発見したときの対処:
- **小さい修正**（1〜2行・リスクなし）: その場で直す
- **影響範囲が広い・判断が必要**: `docs/plan/<PBI>.md` の「発見済みリファクタリング候補」に追記して人間の判断に委ねる
