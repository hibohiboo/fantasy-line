...
ユニットテスト・統合テストとは別に、**PBI の Gherkin シナリオに 1 対 1 で対応する受け入れテスト**を用意する。

### 目的と配置

| 種別 | 目的 | 配置 |
|---|---|---|
| ユニット / 統合テスト | コンポーネントが仕様どおりに動くことを確認する | `tests/small/`・`tests/medium/` |
| **受け入れテスト** | **ユーザーストーリーの受け入れ条件を満たすことを確認する** | **`tests/acceptance/`** |

### 受け入れテストのルール

- ファイル名は `<機能名>.acceptance.test.ts` とする
- 各テストケースのコメントに対応する Gherkin シナリオ番号を明記する
- テストサイズは Medium（DB あり・ブラウザなし）を基本とする
- 実装計画では **ハンドラーテスト完了後・インフラ作業前** に配置する（API が揃ってから一気に通す）

### 受け入れテストが通ることの意味

受け入れテストが全件 GREEN = PBI の受け入れ条件を満たした = レビュー可能な状態。
ユニット・統合テストが通っていても受け入れテストが通っていなければ完成とみなさない。
